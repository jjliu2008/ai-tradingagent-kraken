...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Iterative refinement screener over 50+ targeted strategy variants"
    )
    ...
    parser.add_argument("--summary-csv", default="iterative_meta_summary.csv")
    parser.add_argument("--trades-csv", default="iterative_meta_trades.csv")
    parser.add_argument("--filter-csv", default="iterative_meta_filter_effects.csv")
...
def _config_variant(stop_pct: float, target_pct: float, hold_bars: int, fast_hold_bars: int) -> strat.StrategyConfig:
...
def _build_branch_masks(frame: pd.DataFrame, base_name: str) -> dict[str, pd.Series]:
    candidates = {name: mask for name, mask, _ in adaptive.build_adaptive_candidates(frame, int(frame["base_interval"].iloc[0]))}
    base = candidates[base_name].fillna(False)
    score = _score_frame(frame)
    masks = {
        "base": base,
        "trend_t008": base & (frame["trend_strength"] > 0.008),
        "trend_t009": base & (frame["trend_strength"] > 0.009),
        "vwap_003": base & (frame["distance_from_vwap"] <= 0.03),
        "vwap_0025": base & (frame["distance_from_vwap"] <= 0.025),
        "close_075": base & (frame["close_location"] > 0.75),
        "score_43": base & (score > 43.37),
        "trend_t008__vwap_003": base & (frame["trend_strength"] > 0.008) & (frame["distance_from_vwap"] <= 0.03),
        "trend_t008__vwap_0025": base & (frame["trend_strength"] > 0.008) & (frame["distance_from_vwap"] <= 0.025),
        "trend_t009__vwap_003": base & (frame["trend_strength"] > 0.009) & (frame["distance_from_vwap"] <= 0.03),
        "trend_t008__score_43": base & (frame["trend_strength"] > 0.008) & (score > 43.37),
        "trend_t008__vwap_003__score_43": (
            base
            & (frame["trend_strength"] > 0.008)
            & (frame["distance_from_vwap"] <= 0.03)
            & (score > 43.37)
        ),
        "trend_t008__vwap_003__close_075": (
            base
            & (frame["trend_strength"] > 0.008)
            & (frame["distance_from_vwap"] <= 0.03)
...
def _tokenize(label: str) -> dict[str, str]:
    if "__" not in label:
        return {"branch": label, "mask": "baseline", "exit": "baseline"}
    branch, rest = label.split("::", 1)
    mask_name, exit_name = rest.split("__", 1)
    return {"branch": branch, "mask": mask_name, "exit": exit_name}
...
def main() -> None:
    ...
    coq_frame = frames[3]["COQUSD"].copy().reset_index(drop=True)
    ...
    branch_specs = {
        "giga5_union": _build_branch_masks(giga_frame, "freqai_mtf_adaptive_union"),
        "coq3_momaccel": _build_branch_masks(coq_frame, "freqai_mtf_momentum_accel"),
    }
    branch_frames = {
        "giga5_union": giga_frame,
        "coq3_momaccel": coq_frame,
    }
    branch_pairs = {
        "giga5_union": "GIGAUSD",
        "coq3_momaccel": "COQUSD",
    }
    ...
    for cost_label, side_cost_pct in cost_scenarios:
        print(f"\n{'=' * 112}")
        print(f" ITERATIVE META SCREEN | cost_model={cost_label} | side_cost={side_cost_pct:.4%}")
        print(f"{'=' * 112}")
        ...
        for branch_name, masks in branch_specs.items():
            pair = branch_pairs[branch_name]
            source_frame = branch_frames[branch_name]
            for mask_name, mask in masks.items():
                prepared = _prepare_frame(source_frame, mask)
                for exit_name, config in exit_profiles.items():
                    label = f"{branch_name}::{mask_name}__{exit_name}"
    ...
    for label in summary["label"]:
        token_rows.append(_tokenize(label))
    token_df = pd.DataFrame(token_rows)
    summary = pd.concat([summary.reset_index(drop=True), token_df], axis=1)
    ...
    filter_effects = (
        summary.loc[summary["label"] != "baseline_tc15_tighter_volume_cap"]
        .groupby(["cost_model", "branch", "mask", "exit"], as_index=False)
        .agg(
            trades_full=("trades_full", "mean"),
            net_full=("net_full", "mean"),
            net_recent=("net_recent", "mean"),
            win_full=("win_full", "mean"),
            max_dd=("max_dd", "mean"),
        )
        .sort_values(["cost_model", "net_full", "net_recent"], ascending=[True, False, False])
    )
    ...
    top = summary.groupby("cost_model", group_keys=False).head(15)
    print(top[cols].to_string(index=False, formatters=formatters))

    summary.to_csv(args.summary_csv, index=False)
    filter_effects.to_csv(args.filter_csv, index=False)
    print(f"\nWrote summary to {args.summary_csv}")
    print(f"Wrote filter effects to {args.filter_csv}")
    if trade_rows:
